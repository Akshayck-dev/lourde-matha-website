        ...
        <div ref={sliderRef} className="mt-10 flex snap-x gap-5 overflow-x-auto pb-4 [-ms-overflow-style:none] [scrollbar-width:none] md:overflow-visible md:pb-0 [&::-webkit-scrollbar]:hidden md:grid md:grid-cols-2 md:gap-px md:bg-maroon/10 lg:grid-cols-4">
          {VALUES.map((v, i) => (
            <Reveal key={v.title} delay={i * 0.07} className="h-full w-[80%] shrink-0 snap-start sm:w-[62%] md:w-auto">
              <div className="group h-full rounded-2xl bg-ivory p-9 transition-colors duration-500 hover:bg-maroon-deep">
    ...
    <section className="bg-maroon-deep py-14 md:py-20">
      <div className="mx-auto max-w-5xl px-5 md:px-8">
        ...
        <div className="mt-10 flex justify-center">
          <Reveal>
            <div className="group relative max-w-sm rounded-2xl border border-gold/25 bg-maroon-rich/40 p-10 text-center transition-colors duration-500 hover:border-gold/60">
              {/* monogram */}
          ...
          {SACRAMENTS.map((s, i) => {
            ...
            return (
              <Reveal key={s.name} delay={(i % 3) * 0.07} className="h-full w-[80%] shrink-0 snap-start sm:w-[62%] md:w-auto">
                <div className="group h-full rounded-2xl bg-ivory p-9 transition-colors duration-500 hover:bg-maroon-deep">
                  <span className="flex h-12 w-12 items-center justify-center rounded-xl border border-gold/50 text-gold-dark transition-colors duration-500 group-hover:border-gold/60 group-hover:text-gold-light">
          ...
          {MINISTRIES.map((m, i) => {
            ...
            return (
              <Reveal key={m.name} delay={(i % 3) * 0.07} className="w-[80%] shrink-0 snap-start sm:w-[62%] md:w-auto">
                <div className="h-full rounded-2xl border border-gold/25 bg-maroon-rich/40 p-9 transition-colors duration-500 hover:border-gold/60">

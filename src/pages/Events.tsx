        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2">
          {PATRONAL_FEASTS.map((feast, i) => (
            <Reveal key={feast.name} delay={i * 0.08} className="h-full">
              <div className="flex h-full flex-col rounded-2xl border border-gold/25 bg-maroon-rich/40 p-10 transition-colors duration-500 hover:border-gold/60 md:p-12">
      ...
      {event && (
        ...
        >
          <motion.div
            ...
            className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-ivory shadow-soft"
              ...
              <dl className="mt-6 space-y-4 border-t border-maroon/10 pt-6">
                {[
                  ...
                  <div key={label} className="flex items-center gap-4">
                    <span className="flex h-10 w-10 items-center justify-center rounded-xl border border-gold/50 text-gold-dark">
            ...
            <AnimatePresence mode="popLayout">
              {filtered.map((event) => (
                <motion.article
                  ...
                  className="group flex flex-col overflow-hidden rounded-2xl bg-white/70 shadow-card transition-shadow duration-500 hover:shadow-soft w-[80%] shrink-0 snap-start sm:w-[62%] md:w-auto"
                >
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span className="absolute left-4 top-4 rounded-full bg-maroon-deep/85 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-gold-light backdrop-blur">
                      {event.category}
                    </span>
                  </button>
            ...
            {FEAST_VIDEOS.map((video, i) => (
              <Reveal key={video.url} delay={i * 0.08}>
                <a
                  ...
                  className="group block overflow-hidden rounded-2xl bg-maroon/40 ring-1 ring-gold/25 transition-all duration-300 hover:-translate-y-1 hover:ring-gold/60"
